...
if (isset($_GET['logout'])) {
    unset($_SESSION[FM_SESSION_ID]['logged'], $_SESSION['token']);
...
if ('OFF' != $ip_ruleset) {
    ...
    if ('AND' == $ip_ruleset) {
        if (true == $whitelisted && false == $blacklisted) {
            $proceed = true;
        }
    } elseif ('OR' == $ip_ruleset) {
        if (true == $whitelisted || false == $blacklisted) {
            $proceed = true;
        }
    }
...
if ($use_auth) {
    if (isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']])) {
        ...
        if (function_exists('password_verify')) {
            if (isset($auth_users[$_POST['fm_usr']], $_POST['fm_pwd']) && password_verify($_POST['fm_pwd'], $auth_users[$_POST['fm_usr']]) && verifyToken($_POST['token'])) {
...
$_POST = (false != strpos($input, 'ajax') && false != strpos($input, 'save')) ? json_decode($input, true) : $_POST;
...
if ((isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']]) || !FM_USE_AUTH) && isset($_POST['ajax'], $_POST['token']) && !FM_READONLY) {
    ...
    if (isset($_POST['type']) && 'search' == $_POST['type']) {
    ...
    // backup files
    if (isset($_POST['type']) && 'backup' == $_POST['type'] && !empty($_POST['file'])) {
    ...
    if (isset($_POST['type']) && 'settings' == $_POST['type']) {
        ...
        $hco = isset($_POST['js-hide-cols']) && 'true' == $_POST['js-hide-cols'] ? true : false;
...
if (!empty($_FILES) && !FM_READONLY) {
    ...
    if (is_writable($targetPath)) {
        ...
        if (empty($f['file']['error']) && !empty($tmp_name) && 'none' != $tmp_name && $isFileAllowed) {
            if ($chunkTotal) {
                $out = @fopen("{$fullPath}.part", 0 == $chunkIndex ? 'wb' : 'ab');
                if ($out) {
                    $in = @fopen($tmp_name, 'r');
                    if ($in) {
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-5">
                            <select class="form-select w-100" id="js-3-0" name="js-theme-3">
                                <option value='light' <?php if ('light' == $theme) {
                                    echo 'selected';
                                } ?>><?php echo lng('light'); ?></option>
                                <option value='dark' <?php if ('dark' == $theme) {
                                    echo 'selected';
...
if (isset($_GET['view'])) {
    ...
    if ($online_viewer && 'false' !== $online_viewer && in_array($ext, fm_get_onlineViewer_exts())) {
        ...
        <div class="col-12">
            ...
            <p class="break-word">
                ...
                if ($is_image) {
                    ...
                    echo '<strong>'.lng('Image size').':</strong> '.($image_size[0] ?? '0').' x '.($image_size[1] ?? '0').'<br>';
                }
                // Text info
                if ($is_text) {
    ...
    if ($is_text && !FM_READONLY) {
        ?>
            ...
            <?php
            if ($is_onlineViewer) {
                if ('google' == $online_viewer) {
                    echo '<iframe src="https://docs.google.com/viewer?embedded=true&hl=en&url='.fm_enc($file_url).'" frameborder="no" style="width:100%;min-height:460px"></iframe>';
                } elseif ('microsoft' == $online_viewer) {
    ...
    ?>
                <tr>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY) { ?>
                        ...
                        <?php } ?>
                        <a title="<?php echo lng('DirectLink'); ?>" href="<?php echo fm_enc(FM_ROOT_URL.(FM_PATH != '' ? '/'.FM_PATH : '').'/'.$v.'/'); ?>" target="_blank"><i class="fa fa-link" aria-hidden="true"></i></a>
                    </td>
                </tr>
                <?php
    flush();
...
function fm_get_size($file)
{
    ...
    if (!isset($isdarwin)) {
        $isdarwin = ('DARWIN' == strtoupper(substr(\PHP_OS, 0)));
    }

    static $exec_works;
    if (!isset($exec_works)) {
        $exec_works = (function_exists('exec') && !ini_get('safe_mode') && 'EXEC' == @exec('echo EXEC'));
    }
...
function fm_get_directorysize($directory)
{
    ...
    if (false !== $directory && '' != $directory && file_exists($directory)) {
...
{
    return (false === strpbrk($text, '/?%*:|"<>')) ? true : false;
}
                            ...
                            <div class="input-group-append btn-group">
                                <span class="input-group-text dropdown-toggle brl-0" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"></span>
                                  <div class="dropdown-menu dropdown-menu-right">
                                    <a class="dropdown-item" href="<?php echo $path2 = $path ?: '.'; ?>" id="js-search-modal" data-bs-toggle="modal" data-bs-target="#searchModal"><?php echo lng('Advanced Search'); ?></a>
                                  </div>
                            </div>
                        </div>
                    </li>
...
<?php if (isset($_GET['edit'], $_GET['env']) && FM_EDIT_FILE && !FM_READONLY) {
    $ext = pathinfo($_GET['edit'], \PATHINFO_EXTENSION);
    $ext =  'js' == $ext ? 'javascript' : $ext;
    ?>
