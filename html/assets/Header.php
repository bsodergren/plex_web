...
$files                     = PlexFS::get_filelist($css_dir, 'bootstrap.min.css');
...
foreach ($files as $stylesheet) {
    ...
    // $name =
    $css_html .= Template::GetHTML('base/header/header_css_link', ['CSS_NAME' => $theme, 'CSS_URL' => $stylesheet]);
}
...
// }
$params['SCRIPTS']         = Template::GetHTML('base/header/header_scripts', $params);

if (!defined('VIDEOINFO')) {
    $params['SCRIPTS'] .= Template::GetHTML('base/header/header_filelist', ['__LAYOUT_URL__' => __LAYOUT_URL__]);
} else {
    $params['SCRIPTS'] .= Template::GetHTML('base/header/header_videoinfo', ['__LAYOUT_URL__' => __LAYOUT_URL__]);
}

if (defined('GRID_VIEW')) {
    $params['SCRIPTS'] .= Template::GetHTML('base/header/header_grid', ['__LAYOUT_URL__' => __LAYOUT_URL__]);
}
