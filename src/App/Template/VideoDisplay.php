...
use Plex\Utilities\PlexStrings;
/**
 * plex web viewer
 */
...
class VideoDisplay
{
    ...
    public function fileRow($params, $field, $value, $class, $id = '')
    {
        ...
        if (defined('NONAVBAR')) {
            if ('' != $id) {
                ...
                $params['VIDEOINFO_EDIT_JS'] .= Template::GetJS(
        ...
        $params['FIELD_ROW_HTML'] .= Template::GetHTML(
            'videoinfo/file_row',
            [
                'FIELD'     => $field,
                'VALUE'     => $value,
    ...
    public function fileInfo($fileInfoArray, $total_files)
    {
        ...
        $params['ROW_ID']             = '';
        if (!defined('NONAVBAR')) {
            $params['FILE_NAME']     = Template::GetHTML(
            ...
            $params['VERTICAL_TEXT'] = Template::GetHTML(
        ...
        $params['DELETE_ID']          = Render::add_hidden('id', $row_id);
        ...
        foreach ($fileInfoArray as $key => $value) {
            ...
            switch ($key) {
                ...
                case 'duration':
                    $params                       = $this->fileRow($params, 'Duration', PlexStrings::videoDuration($value), $class);
                    ++$x;
                ...
                case 'substudio':
                    if ('' != $value || defined('NONAVBAR')) {
                        if (!defined('NONAVBAR')) {
                            $value = HTMLDisplay::keyword_list($key, $value);

                            // $value = Template::GetHTML(
                ...
                case 'filesize':
                    $params                       = $this->fileRow($params, ucfirst($key), PlexStrings::display_size($value), $class);
                    ++$x;
        ...
        if (true == $this->showVideoDetails) {
            $params = $this->fileRow($params, '', Template::GetHTML(
                $this->template_base.'/file_videoinfo',
                $infoParams
            ), $class);
        }

        $table_body_html['filecards'] = Template::GetHTML($this->template_base.'/file', $params);
    ...
    public function fileList($results, $option = '', $page_array = [])
    {
        ...
        foreach ($results as $id => $row) {
            ...
            $table_html .= Template::GetHTML($this->template_base.'/file_form_wrapper', [
                'FILE_ID'         => $row_id,
        ...
        // $javascript_html = Template::GetHTML(
